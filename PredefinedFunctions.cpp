...
PredefinedFunctions::PredefinedFunctions() {
	...
	*/
	...
	addBuiltinFunction("ifthen", [](Runner* r) {
							...
							if (truthyTailCall) {
								...
								while (1) {
									...
									if (sgn(r->getCurrentStack().pop().numberValue.integerValue) < 1) {
										r->runList(falsy.literalFunctions);
										return;
									}
								}
							} else {
